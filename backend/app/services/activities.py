...
def get_my_activities(db: Session, user_id: UUID, limit: int = 20, skip: int = 0):
    # Filter di SQL: expense yang dibayar user ATAU user punya split di dalamnya
    expenses = (
        db.query(Expense)
        .options(
    ...
    for exp in expenses:
        ...
        amount_display = float(exp.amount) if paid_by_me else float(split_row.amount_owed)
